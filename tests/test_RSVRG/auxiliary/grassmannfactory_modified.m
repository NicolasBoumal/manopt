function M = grassmannfactory_modified(n, p, k)
...
%       (this only matters if k>1).
    ...
    % Parallel translation: HK
    % BM: updated to handle multiple Grassmann manifolds.
    M.paratransp = @parallel_translation;
    function zeta = parallel_translation(X, H, eta)
        zeta = zeros(n, p, k);
        for i = 1 : k
            x = X(:, :, i);
            h = H(:, :, i);
