function M = stiefelgeneralizedfactory_HS_KA(n, p, B)
...
%%%%%%%%%%%%%%%%%
% Cholesky QR-based retraction by H. Sato and K. Aihara.
    M.retr = @QRretraction;
    function Y = QRretraction(X, U, t)
        if nargin < 3
            t = 1.0;
        end
        Y = gqf(X + t*U); % Ensure that Y'*B*Y is identity.
    end
