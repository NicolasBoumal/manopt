...
% (the empty matrix).
%
% This algorithm is /not/ recommended for any purposes. The implementation
% was not tested on practical problems, and the computation of the
% centroids as Karcher means seems overly expensive for the purpose of
% derivative free optimization anyway. At best, this is a starting point
% for someone who would like to make something nice out of it.
%
...
% Change log: 
    ...
    while true
        ...
        % Sort simplex points by cost (this is not counted in the iteration
        % cost because a careful implementation does not need a full sort
        % at each iteration)
        [costs order] = sort(costs);
        x = x(order);
        ...
        % If the reflected point is worse than our second to worst point,
        % contract. (TODO: This if condition is always satisfied, no?)
