...
% function X = dominant_invariant_subspace(A, p)
%
% Input: A symmetric matrix A of size nxn and an integer p < n.
% Output: An orthonormal matrix X of size nxp such that trace(X'*A*X) is
%         maximized. That is, the columns of X form an orthonormal basis of
%         a dominant subspace of dimension p of A. These are thus
...
%   
    ...
    % solution are positve, i.e., we find an isolated minimizer. If we
    ...
    % cost(XQ) = cost(X) for all Q orthogonal, then we should see
