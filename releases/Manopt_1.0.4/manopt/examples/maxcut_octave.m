...
%    now HAVE TO (in Octave) take them as input/output. Apparently, this
%    should change in Octave.
%  * You can't define those functions as nested functions (which is so so
%    comfortable and elegant though :( )
%
% These limitations stem from the following limitations of Octave (don't
% get me wrong: they do an amazing job; it's just not 100% Matlab-like):
%  * No nested functions support
%  * No nargin/nargout for user-supplied functions or inline functions
% Here are other discrepancies we had to take into account when adapting
% Manopt:
%  * No Java classes, so the hashmd5 privatetool was adapted.
%  * No 'import' packages: the whole structure of the toolbox changed...
...
%  * try/catch blocks don't give the catch an exception object.
%  * no minres function; using gmres instead, which is kind of a waste.
%
% See also: maxcut
