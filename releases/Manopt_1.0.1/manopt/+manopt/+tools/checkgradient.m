...
% Change log: 
    ...
    if isfield(problem.M, 'proj')
        grad = getGradient(problem, x, dbstore);
        if isfield(problem.M, 'tangent2ambient')
            agrad = problem.M.tangent2ambient(x, grad);
        else
            agrad = grad;
        end
        pgrad = problem.M.proj(x, agrad);
        ...
        fprintf(['Unfortunately, Manopt was un able to verify that the '...
                 'gradient is indeed a tangent vector. Please verify ' ...
                 'this manually.']);
    end
