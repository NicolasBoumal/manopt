...
% assumed to be (noisy) rotated versions of a reference cloude Atrue.
...
%   
    ...
    if ~exist('A_measure', 'var')
        ...
        % rotated version of a reference cloud A. Rotations are unfiromly
    ...
    % is used to keep the 3D matrix E in memory, as it is also used in the
    % computation of the gradient and of the Hessian. This way, we prevent
    ...
    % Riemannian fradient of the cost function.
    ...
    % Call a solver on our problem.
    % TODO: find a clever initial guess to start the optimization.
    ...
    % solution would be to work on a quotient manifold.
    if M.dim() <= 512
